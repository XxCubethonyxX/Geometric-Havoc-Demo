...
class Bar extends FlxSpriteGroup
{
	...
	public function new(x:Float, y:Float, type:String, image:String = 'healthBar', valueFunction:Void->Float = null, boundX:Float = 0, boundY:Float = 1)
	{
		super(x, y);
		...
		switch(type)
		{
			case 'default':
				...
				add(bg);
			case 'png':
				leftBar = new FlxSprite().loadGraphic(Paths.image(image));
				leftBar.color = FlxColor.WHITE;
				leftBar.antialiasing = true;
				
    			rightBar = new FlxSprite().loadGraphic(Paths.image(image ));
				rightBar.color = FlxColor.BLACK;
				rightBar.antialiasing = true;
				barWidth = Std.int(rightBar.width);
				barHeight = Std.int(rightBar.height);
				add(leftBar);
				add(rightBar);
	...
	public function regenerateClips()
	{
		if(leftBar != null)
		{
			if(type == 'default')leftBar.setGraphicSize(Std.int(bg.width), Std.int(bg.height));
			leftBar.updateHitbox();
			leftBar.clipRect = new FlxRect(0, 0, Std.int(leftBar.width), Std.int(leftBar.height));
		}
		if(rightBar != null)
		{
			if(type == 'default')rightBar.setGraphicSize(Std.int(bg.width), Std.int(bg.height));
