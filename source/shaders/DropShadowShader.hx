...
{
    var uv = frame.uv; // FlxUVRect (abstract over FlxRect)

    // Compute the real bounds
    var left   = uv.left;          // uv.x
    var top    = uv.right;         // uv.y
    var right  = uv.left + uv.top; // x + width?
    var bottom = uv.right + uv.bottom; // y + height?
