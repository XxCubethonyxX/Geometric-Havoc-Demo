...
using Lambda;
using utility.IteratorTools; //pslice shit 
using StringTools;
...
class StickerSubState extends MusicBeatSubstate
{
  ...
  public function new(?oldStickers:Null<Array<StickerSprite>>, ?targetState:StickerSubState->FlxState):Void
  {
  ...
  public function degenStickers():Void
  {
    ...
    for (ind => sticker in grpStickers.members)
    {
      ...
      new FlxTimer().start(grpStickers.members[ind].timing, _ -> {

       
        grpStickers.members[ind].visible = false;
        ...
        {
          trace("Sticker degen complete, closing substate.");
          switchingState = false;
  ...
  function regenStickers():Void
  {
    ...
    stickers = infoObj;
    trace("Sticker info: " + infoObj.name + " by " + infoObj.artist);
